...
**Replaceable by Design**: Modularity makes systems flexible, replaceable, and understandable. Design each part to do one thing well, interact only through well-defined boundaries, and stay simple enough to swap, extend, or recombine without ripple effects. ^modular-design-principles-definition

- **Loose Coupling, Tight Cohesion**: Maximize internal relatedness within modules (cohesion) while minimizing dependencies between them (coupling). Group related functionality together; keep unrelated functionality separate. This localizes changes, reduces ripple effects, and bounds reasoning to one module at a time. ^loose-coupling-tight-cohesion
...
- **Replaceable Parts**: Design components so you can swap them using only their interfaces. ^replaceable-parts
...
**Shape Data, Shape System:** Data is the foundation of system reliability. Clean models make code simple, predictable, and resilient. Define strong primitives, capture relationships explicitly, and enforce invariants in the data itself to reduce logic overhead and prevent entire classes of errors. ^data-first-principles-definition
...
### Data Fit & Stability
- **Access-Pattern Fit**: Choose structures aligned with expected reads and writes. ^access-pattern-fit
...
- **Behavior as Data**: Represent rules and configs declaratively instead of branching logic. ^behavior-as-data
...
- **Strategy Subfolders**: Extract variants into subfolders when using strategy or plugin patterns: `outputStyles/` for format-specific templates, `parseStrategies/` for language-specific parsers. ^strategy-subfolders
...
- **Simplicity First**: Make interfaces as simple as possible to implement. Favor simple designs that reduce complexity and prevent errors. Prefer one good way over multiple complex options. ^simplicity-first
- **Progressive Defaults**: Design interfaces with sensible defaults that handle 80% of use cases, while providing clear customization paths for specialized needs. This reduces initial complexity, accelerates adoption, and keeps common paths simple while retaining flexibility for power users. ^progressive-defaults
...
**Minimum for Momentum**: MVPs prove concepts. The goal: validate ideas quickly, minimize wasted effort, and stop building beyond what's required. Keep scope tight, solutions simple, and lean on existing foundations so you can adapt fast. ^mvp-principles-definition
...
- **Implement When Needed**: Implement features only when necessary to prevent over-engineering. ^implement-when-needed
...
**Consistency from Tools, Flexibility from LLMs**: LLMs are powerful but unreliable at rigid work; deterministic tools are fast but brittle at semantics. Diversify execution by offloading predictable tasks to tools and keeping LLMs for judgment, maximizing strengths while reducing weaknesses. ^deterministic-offloading-principles-definition

- **Mechanical Separation**: Route deterministic tasks (I/O, parsing, validation, search, transforms) to tools; reserve LLMs for semantic work (intent, design choices, contextual content). ^mechanical-separation
...
- **Tool-First Design**: Use or build specialized tools for repetitive operations instead of LLM brute force. ^tool-first-design
...
- **Descriptive Labels**: Distinguish system scope, operation type, or intended outcome without requiring documentation lookup. ^descriptive-labels
...
- **Selective Documentation**: Document all class-level architecture and public APIs with JSDoc including `@param` and `@returns`. Document complex private methods with multi-step algorithms or important behavioral notes. Use inline comments for simple utilities. ^selective-documentation
...
- **Over-Engineered Structures**: Avoid exotic data models before you need them. ^over-engineered-structures
- **Leaky Flags**: Avoid ambiguous enums or flags that require out-of-band knowledge to interpret. ^leaky-flags
