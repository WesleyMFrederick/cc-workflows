...
```text
NewMCPServer()
  → create server instance                          [OBS] server.go:98-107
  → create conversationsHandler                     [OBS] server.go:109
  → for each tool: shouldAddTool() × { register | skip }  [OBS] server.go:111-278
  → ⚠ users_search: s.AddTool() directly (NO shouldAddTool gate)  [OBS] server.go:281-293
  → create channelsHandler, usergroupsHandler       [OBS] server.go:295-296
  → for each channels/usergroups tool: shouldAddTool() × { register | skip }  [OBS] server.go:298-414
  → register resources (channels, users)            [OBS] server.go:444-456
```

**Key observation:** Every tool except `users_search` goes through `shouldAddTool()`. Lines 281-293 call `s.AddTool()` directly with a hardcoded string `"users_search"` — no constant, no gating. [OBS] `server.go:281`

### Trace 2: users_search Tool Invocation (OAuth path)

```text
MCP client calls "users_search" with {query, limit}
  → middleware chain: errorRecovery → logger → auth   [OBS] server.go:104-106
  → UsersSearchHandler()                              [OBS] conversations.go:342
    → apiProvider.IsReady() check                     [OBS] conversations.go:345-348
    → parseParamsToolUsersSearch(request)              [OBS] conversations.go:350
      → validate query non-empty                      [OBS] conversations.go:1008-1011
      → clamp limit to 1-100, default 10              [OBS] conversations.go:1013-1019
    → apiProvider.SearchUsers(ctx, query, limit)      [OBS] conversations.go:361
      → IsOAuth() == true                             [OBS] api.go:1106
      → searchUsersInCache(query, limit)              [OBS] api.go:1107
        → check usersReady                            [OBS] api.go:1116-1118
        → compile case-insensitive regex from query   [OBS] api.go:1120
        → iterate usersSnapshot map                   [OBS] api.go:1127
        → match against: Name, RealName, DisplayName, Email  [OBS] api.go:1132-1135
        → skip deleted users                          [OBS] api.go:1129-1130
        → return up to limit results                  [OBS] api.go:1138-1139
    → ProvideChannelsMaps()                           [OBS] conversations.go:367
    → for each result: scan channels for IM match to get DM channel ID  [OBS] conversations.go:376-381
    → build UserSearchResult CSV                      [OBS] conversations.go:383-391
    → return CSV text or "No users found"             [OBS] conversations.go:394-404
```
...
(Same as Trace 2 through parseParams)
    → apiProvider.SearchUsers(ctx, query, limit)      [OBS] conversations.go:361
      → IsOAuth() == false                            [OBS] api.go:1106
      → client.UsersSearch(ctx, query, limit)         [OBS] api.go:1110
        → edge Client.UsersSearch()                   [OBS] edge/users.go:27
          → callEdgeAPI("users/search", {query, count})  [OBS] edge/users.go:41
          → return resp.Results                       [OBS] edge/users.go:49
...
CLI startup parses SLACK_MCP_ENABLED_TOOLS env var
  → ValidateEnabledTools(tools)                       [OBS] server.go:59-77
    → builds set from ValidToolNames                  [OBS] server.go:60-63
    → checks each requested tool against set          [OBS] server.go:66-69
    → ⚠ "users_search" NOT in ValidToolNames          [OBS] server.go:43-57
    → if user includes "users_search" → error: "invalid tool name"  [F-LK-1]
```
...
- Constants defined: 13 (`ToolConversationsHistory` through `ToolUsergroupsUsersUpdate`) [OBS] `server.go:27-41`
...
<!-- /group-id:response-202602181200 -->
- ValidToolNames entries: 13 (same 13) [OBS] `server.go:43-57`
- Actual tools registered: 14 (13 + users_search) [OBS] `server.go:111-293`
- **Gap: 1 tool registered without constant or ValidToolNames entry** [M]

**\[M-2\]: shouldAddTool usage**
- Tools using shouldAddTool gate: 13/14 [OBS] `server.go:111-414`
- Tools bypassing shouldAddTool: 1 (users_search) [OBS] `server.go:281`
...
> Conclusions about the current system derived from combining observations [OBS] and measurements [M]. The delta references these to justify what needs to change.

**[F-LK-1]: SLACK_MCP_ENABLED_TOOLS will reject users_search**
...
**[F-LK-2]: users_search cannot be disabled**
...
**[F-LK-3]: Commit message is misleading about token support**
...
**[F-ID-4]: DM channel lookup is O(n) per result**
...
**[C-1]:** Must match existing tool registration pattern — constant, ValidToolNames entry, shouldAddTool wrapper [OBS] `server.go:27-57, 111-278`^c1
- Breaking this convention means the PR will be rejected — every other tool follows this pattern

**[C-2]:** Must maintain backward compatibility — tool name stays `"users_search"`, params stay `{query, limit}` [OBS] already deployed in flare576's PR#179 [https://github.com/korotovsky/slack-mcp-server/pull/179](https://github.com/korotovsky/slack-mcp-server/pull/179)^c2
- Renaming the tool or changing params would break any MCP client already using the merged implementation

**[C-3]:** PR must target upstream `korotovsky/slack-mcp-server` repo and reference issue #205 [OBS] from original request ^c3
- Wrong PR target means the fix never reaches the npm package and issue #205 stays open

**[C-4]:** Go project — changes must follow existing code style (gofmt, existing patterns) [OBS]^c4
...
**[A-1]:** ~~The upstream maintainer will accept a polish/integration-fix PR.~~ **UPGRADED → Observation [OBS]:**^a1
- No CONTRIBUTING.md exists, but repo actively merges external PRs — 10 recent merged PRs from 6 different contributors (Flare576, RedSlowpoke, aminsaedi, ljagiello, dalley, dosentmatter) [OBS]
- MIT license. No formal PR policy — just submit and maintainer reviews [OBS]
...
- Some tools require a special env var to register (e.g. `conversations_add_message` needs `SLACK_MCP_ADD_MESSAGE_TOOL` set) [OBS] `server.go:161,183,202,221`
- This is the `envVarName` param in `shouldAddTool()` — when non-empty, the tool only registers if that env var is set [OBS] `server.go:90-93`
- Read-only tools like `channels_list` pass `""` (no env var needed) [OBS] `server.go:298`
...
- Tests use OpenAI SDK (`gpt-4.1-mini`) sending natural language prompts to MCP server via SSE/ngrok [OBS] `conversations_test.go:47-111`
- Requires: `SLACK_MCP_OPENAI_API` env var, ngrok, and a Slack workspace with known test data [OBS]
...
**[A-4]:** ~~The npm package `slack-mcp-server@latest` may not include `users_search` or `usergroups_*` tools yet.~~ **UPGRADED → Observation [OBS]:**^a4
- npm `slack-mcp-server@latest` is `v1.1.28`. Git tag `v1.1.28` has ~60 commits before master HEAD [OBS]
- `users_search` (PR #179), usergroups (PR #193), enabled-tools refactor (PR #173) all merged after `v1.1.28` [OBS]
- Claude Code runs `npx slack-mcp-server@latest` (npm), not the local git clone — confirmed in `.mcp.json:26-32` [OBS]
- **Fix:** ~~Switch `.mcp.json` to local build~~ **APPLIED** — `.mcp.json` now points to local binary at `slack-mcp-server/build/slack-mcp-server` (`v1.1.28-82-g6ddc828`, 82 commits ahead of npm). Requires Claude Code restart to take effect [OBS]
...
- `SLACK_MCP_ENABLED_TOOLS` env var: comma-separated list of tool names to enable. If unset, all tools register (minus env-gated ones). If set, only listed tools register via `shouldAddTool()` [OBS] `server.go:79-96`
- Per-tool env vars (second gate): some tools require a specific env var to exist regardless of `SLACK_MCP_ENABLED_TOOLS`:
  - `SLACK_MCP_ADD_MESSAGE_TOOL` → `conversations_add_message` [OBS] `server.go:161`
  - `SLACK_MCP_REACTION_TOOL` → `reactions_add`, `reactions_remove` [OBS] `server.go:183,202`
  - `SLACK_MCP_ATTACHMENT_TOOL` → `attachment_get_data` [OBS] `server.go:221`
- Tools with no env var gate (pass `""`) always register when `SLACK_MCP_ENABLED_TOOLS` is unset or includes them: `conversations_history`, `conversations_replies`, `conversations_search_messages`, `channels_list`, all `usergroups_*` tools [OBS]
- `conversations_search_messages` has an additional gate: only registers for non-bot tokens [OBS] `server.go:277`
...
|------|----------------|-------------|
| `reactions_add` | `SLACK_MCP_REACTION_TOOL` | Add an emoji reaction to a message (destructive) [OBS] `server.go:183-200` |
| `reactions_remove` | `SLACK_MCP_REACTION_TOOL` | Remove an emoji reaction from a message (destructive) [OBS] `server.go:202-219` |
| `attachment_get_data` | `SLACK_MCP_ATTACHMENT_TOOL` | Download attachment by file ID, max 5MB (read-only) [OBS] `server.go:221-231` |

**No env var gate (should register automatically — if missing, likely npm package version lag):**

| Tool | Description |
|------|-------------|
| `usergroups_list` | List all @mention groups in workspace. CSV: id, name, handle, description, user_count, is_external (read-only) [OBS] `server.go:322-338` |
| `usergroups_me` | Manage your own group membership: list, join, or leave (mixed read/write) [OBS] `server.go:342-353` |
| `usergroups_create` | Create a new @mention group with name, handle, description, channels (destructive) [OBS] `server.go:356-374` |
| `usergroups_update` | Update group metadata: name, handle, description, channels. Does NOT change members (destructive) [OBS] `server.go:377-398` |
| `usergroups_users_update` | Replace ALL members of a group. WARNING: completely replaces member list (destructive) [OBS] `server.go:400-414` |
| `users_search` | Search users by name, email, or display name. Returns details + DM channel ID. **Ungated — always registers** (read-only) [OBS] `server.go:281-293` |