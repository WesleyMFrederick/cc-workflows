...
In this chapter, we'll start by discussing the principles of DDD and by showing how they can be applied to a particular domain. DDD is a large topic, so we won't be exploring it in detail (for more detailed information on DDD, visit dddcommunity.org<sup>1</sup> ). However, by the end of this chapter you should at least have a good idea of how domain-driven design works and how it is different from database-driven design and object-oriented design.
...
<sup>1.</sup> [http://dddcommunity.org](../../../../../dddcommunity.org/default.htm)
...
<sup>2.</sup> [http://eventstorming.com](../../../../../eventstorming.com/default.htm)
...
For example, in an alternate world, Widgets Inc might already have installed a software package that did order taking and billing together in one system. If you needed to integrate with this legacy system, you would probably need to treat it as a single bounded context, even though it covers multiple domains as shown in the figure [on page 18.](#page-15-0)
...
In the same way, a context map shows the various bounded contexts and their relationships at a high level. The goal is not to capture every detail but to provide a view of the system as a whole. For example, this is what we have so far for the order-taking system as shown in the figure [on page 20.](#page-17-0)
...
The construction of the ubiquitous language is not a one-way process dictated by the domain expert, it is a collaboration between everyone on the team. Nor should you expect the ubiquitous language to be static: it's always a work in progress. As the design evolves, be prepared to discover new terms and new concepts, and let the ubiquitous language evolve correspondingly. We'll see this happen in the course of this book.
